...
export default{
    ...
    methods: {
        ...
        async showTobeList() {
            ...
            const {data: res} = await this.$http.post('showLendingBook',this.bookInfo)
            this.tobeList = []
            if(res.status==200){
        ...
        async renewBook(index){
            const {data: res} = await this.$http.post('renewBook',{user_id: this.userid,book_id: this.tobeList[index].book_id})
            if(res.status==200){
                const confirmResult = await this.$confirm("您是否确定续借本书本书?", "提示", {
                ...
                this.$message.error("您已经续借过一次无法再次续借！")
            }
            else if(res.status==410){
                this.$message.error("您还有未支付的罚款，暂时无法续借，请及时归还超期图书并支付罚款！")
            }
        },
        ...
        async viewdetail(index) {
            ...
            this.queryInfo.bName = this.tobeList[index].bName
            const {data: res} = await this.$http.post('searchBook',this.queryInfo)
            res.bookList[0].book_index = this.tobeList[index].book_id
